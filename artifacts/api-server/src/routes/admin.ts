...
function pawaPayOperationEntries(value: unknown): Array<{ operation: string; status?: string }> {
  ...
  if (value && typeof value === "object") {
    return Object.entries(value as Record<string, unknown>).map(([operation, details]) => ({
      operation,
      status: details && typeof details === "object" && typeof (details as Record<string, unknown>).status === "string"
        ? String((details as Record<string, unknown>).status)
        : undefined,
    }));
  }
...
router.get("/pawapay/status", async (_req, res) => {
  try {
    ...
    res.status(500).json({
      error: "InternalError",
      message: "Impossible de lire l’état pawaPay",
      environment: process.env.PAWAPAY_ENV === "production" ? "production" : "sandbox",
      configured: Boolean(process.env.PAWAPAY_API_TOKEN),
    });
  }
...
router.post("/pawapay/sync-services", async (req: AuthRequest, res) => {
  try {
    ...
    if (discovered.length === 0) {
      throw new Error("La configuration pawaPay ne contient aucun service opérationnel compatible");
    }

    // Only replace the active snapshot after the response has been parsed
    // successfully, so an upstream schema change cannot disable every route.
...
router.get("/env-check", (req: AuthRequest, res) => {
  ...
  const mavianceBaseUrl = process.env.MAVIANCE_ENV === "production"
  ...
  res.json({
    ...
    mavianceEnv: process.env.MAVIANCE_ENV ?? "staging (défaut)",
    providers: {
      maviance: {
        ready: mavianceReady,
        ...
        callbackUrl: process.env.MAVIANCE_IPN_BASE_URL
          ? `${process.env.MAVIANCE_IPN_BASE_URL}/api/ipn/maviance`
          : null,
      },
      pixpay: {
        callbackUrl: process.env.PIXPAY_IPN_BASE_URL
          ? `${process.env.PIXPAY_IPN_BASE_URL}/api/ipn/pixpay`
          : null,
      },
    },
    keys: result,
