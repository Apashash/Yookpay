    ...
    if (isSuccess) {
      ...
      await db
        ...
        .set({
          ...
          metadata: { ...(tx.metadata as object), nowpaymentsStatus: status, actuallyPaid, completedAt: new Date().toISOString() },
          updatedAt: new Date(),
        })
      ...
      await db
        ...
        .set({
          status: "FAILED",
          providerReference: paymentId,
          metadata: { ...(tx.metadata as object), nowpaymentsStatus: status, failedAt: new Date().toISOString() },
          updatedAt: new Date(),
        })
        .where(eq(transactionsTable.id, tx.id));
